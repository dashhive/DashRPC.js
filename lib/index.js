...
function RpcClient(opts) {
  ...
  rpc.protocol = opts.protocol === 'http' ? http : https;
  rpc.batchedCalls = null;
  rpc.disableAgent = opts.disableAgent || false;
  rpc.onconnected = opts.onconnected;

  const isRejectUnauthorized = typeof opts.rejectUnauthorized !== 'undefined';
  rpc.rejectUnauthorized = isRejectUnauthorized ? opts.rejectUnauthorized : true;
...
RpcClient.prototype._enqueue = function (request, callback) {
  ...
  const task = function (taskCallback) {
    ...
    rpc.request(request, newCallback);
  };
...
RpcClient.prototype.request = function (request, callback) {
  ...
  const options = {
    ...
    method: 'POST',
    port: rpc.port,
    rejectUnauthorized: rpc.rejectUnauthorized,
    agent: rpc.disableAgent ? false : undefined,
  };

  if (rpc.timeout) {
    options.timeout = rpc.timeout;
  }
  ...
  const req = rpc.protocol.request(options, (res) => {
    let buf = '';
    res.on('data', (data) => {
      buf += data;
    });
    ...
    res.on('end', () => {
      ...
      try {
        ...
        rpc.log.err(e.stack);
        rpc.log.err(buf);
        rpc.log.err(`HTTP Status code:${res.statusCode}`);
