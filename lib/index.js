...
function RpcClient(opts) {
  ...
  let callNext = queue(queueSize);
  this.queue = {
    push: async function (task) {
      let taskAsync = promisify(task);
      await callNext(taskAsync);
    },
  };
}
...
function queue(limit) {
  ...
  async function launch() {
    ...
    // some tasks may be synchronous, so we
    // must add to the actives queue first
    actives.push(p);

    // wait for and remove finished tasks
